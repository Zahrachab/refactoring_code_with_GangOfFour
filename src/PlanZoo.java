...
public class PlanZoo extends JFrame {
	...
	public void placerEnclos(TreeSet<Enclos> enclos){
		...
		JButton b=null; int j; 
		while(it.hasNext()){
			Enclos e = it.next();
			if (e instanceof Cage){
				j = ProchainePlace(indicePrincipale);
				e.setAdr(j);
				b=cases.get(j);b.setBackground(new Color(139, 69, 19));
				...
				j = ProchainePlace(indicePrincipale);
				e.setAdr(j);
				b=cases.get(j);b.setBackground(Color.YELLOW); b.setForeground(Color.BLACK);
				for( ActionListener al : cases.get(j).getActionListeners() ) 
					cases.get(j).removeActionListener( al );
				cases.get(j).addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
				...
				j = ProchainePlace(indicePrincipale);
				e.setAdr(j);
				b=cases.get(j);b.setBackground(Color.BLUE);
				for( ActionListener al : cases.get(j).getActionListeners() ) 
					cases.get(j).removeActionListener( al );
				cases.get(j).addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
				...
				j = ProchainePlace(indicePrincipale);
				e.setAdr(j);
				b=cases.get(j);b.setBackground(Color.GREEN);
				for( ActionListener al : cases.get(j).getActionListeners() ) 
					cases.get(j).removeActionListener( al );
				cases.get(j).addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
				...
				j = ProchainePlace(indicePrincipale);
				e.setAdr(j);
				b=cases.get(j);b.setBackground(Color.ORANGE);b.setForeground(Color.BLACK);
				for( ActionListener al : cases.get(j).getActionListeners() ) 
					cases.get(j).removeActionListener( al );
				cases.get(j).addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
	...
	public static int ProchainePlace(int principale){
		int resultat=0, i = 1; boolean boo1, boo2;
		while ((i < cases.size()/2)){ boo1 = true; boo2 = true;
